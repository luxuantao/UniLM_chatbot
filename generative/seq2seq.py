...
class Seq2SeqModel(nn.Module):
    ...
    def compute_loss(self, predictions, labels, target_mask):
        ...
        predictions [bs, seq_len - 1, vocab_size] 不包含最后的
        labels [bs, seq_len - 1] 不包含最前面的
        target_mask [bs, seq_len - 1] 不包含最前面的
        """
        predictions = predictions.view(-1, self.vocab_size)  # [bs*(seq_len - 1), vocab_size]
        labels = labels.view(-1)  # [bs*(seq_len - 1)]
        target_mask = target_mask.view(-1)  # [bs*(seq_len - 1)]
        loss = nn.CrossEntropyLoss(ignore_index=0, reduction="none")
        return (loss(predictions, labels) * target_mask.float()).sum() / target_mask.float().sum()  # 通过mask 取消 pad 和句子a部分预测的影响
    ...
    def forward(self,
        ...
        a_mask = ones.tril().float()  # 下三角矩阵 (1, 1, seq_len, seq_len)
        s_ex12 = token_type_id.unsqueeze(1).unsqueeze(2).float()  # (bs, 1, 1, seq_len)
        s_ex13 = token_type_id.unsqueeze(1).unsqueeze(3).float()  # (bs, 1, seq_len, 1)
        a_mask = (1 - s_ex12) * (1 - s_ex13) + s_ex13 * a_mask  # (bs, 1, seq_len, seq_len)
        enc_layers, pooled_output, attention_layers = self.bert(input_tensor,
                                  position_ids=position_enc,
                                  token_type_ids=token_type_id,
                                  attention_mask=a_mask,  # 0的地方不要
                                  output_all_encoded_layers=True)
        squence_out = enc_layers[-1]  # 取出来最后一层输出 [bs, seq_len, hidden_size]
        logits = self.decoder(squence_out)  # 取出来最后一层输出 [bs, seq_len, vocab_size]
        ...
        if train:
            ...
            target_mask = token_type_id[:, 1:].contiguous()  # [bs, seq_len - 1]
            labels = input_tensor[:, 1:].contiguous()  # [bs, seq_len - 1]
            loss = self.compute_loss(logits, labels, target_mask)
            return enc_layers, logits, loss, attention_layers
        else:
            return logits

    def generate(self, text, out_max_length=30, beam_size=3, is_cuda=True):
        # 对 一个 句子生成相应的结果
        ...
        token_ids = torch.tensor(token_ids, device=device).view(1, -1)  # [1, seq_len]
        token_type_ids = torch.tensor(token_type_ids, device=device).view(1, -1)  # [1, seq_len]
        ...
        out_puts_ids = self.greedy_search(token_ids,
                                        token_type_ids,
                                        self.word2ix,
                                        is_cuda=is_cuda)
    ...
    def beam_search(self,
        ...
        output_ids = [[]]
        # 用来保存累计得分
        output_scores = torch.zeros(token_ids.shape[0], device=device)  # [1]
        for step in range(self.out_max_length):
            scores = self.forward(token_ids, token_type_ids, is_cuda=is_cuda, train=False)  # [bs=1, seq_len, vocab_size]
            # print(scores.shape)
            if step == 0:
                # 重复beam-size次 输入ids
                token_ids = token_ids.view(1, -1).repeat(beam_size, 1)  # [beam_size, seq_len]  每一行都一样
                token_type_ids = \
                    token_type_ids.view(1, -1).repeat(beam_size, 1)
            # 计算log 分值 (beam_size, vocab_size)
            logit_score = torch.log_softmax(scores, dim=-1)[:, -1]  # [bs=1, vocab_size]
            logit_score = output_scores.view(-1, 1) + logit_score  # 累计得分 [bs=1, vocab_size]
            # 取topk的时候我们是展平了然后再去调用topk函数
            # 展平
            logit_score = logit_score.view(-1)  # [vocab_size]
    ...
    def greedy_search(self,
        ...
        for step in range(self.out_max_length):
            scores = self.forward(token_ids, token_type_ids, is_cuda=is_cuda, train=False)  # [bs=1, seq_len, vocab_size]
            idx = scores[0][-1].argmax().item()
            if idx == sep_id:
                break
            output_ids.append(idx)
            next_chars = torch.tensor(idx, dtype=torch.long, device=device).view(-1, 1)
            token_ids = torch.cat((token_ids, next_chars), dim=1)
            next_token_type_ids = torch.tensor(1, dtype=torch.long, device=device).view(-1, 1)
