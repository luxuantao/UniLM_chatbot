...
import faiss  # conda install -c conda-forge faiss=1.6.5=py38h60a57df_0_cuda
...
def wam(sentence, w2v_model):
    ...
    for s in clean(sentence).split():
        if s not in w2v_model.wv.key_to_index.keys():  # https://github.com/RaRe-Technologies/gensim/wiki/Migrating-from-Gensim-3.x-to-4
...
class HNSW(object):
    ...
    def search(self, text, k=5):
        ...
        test_vec = wam(clean(text), self.w2v_model)  # [1,300]
        test_vec = test_vec.astype('float32')
        # vecs is a n2-by-d matrix with query vectors
        D, I = self.index.search(test_vec, k)  # D [num=1,k] 距离; I [num=1,k] 下标
        # print(I)
