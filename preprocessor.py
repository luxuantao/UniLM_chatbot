...
def read_file(path, is_train=False):
    ...
    with open(path, 'r') as f:
        ...
        for lines in f:
            ...
            if session_id != session_id_in_doc and session_id != '':
                ...
                if fc != '':
                    tmp.append([session_id, 'custom' if str(is_assistance) == '0' else 'assistance', fc])
                    content = []
                chat.extend(tmp)
                tmp = []
    ...
    if content != '':
        tmp.append([session_id, 'custom' if str(is_assistance) == '0' else 'assistance', filter_content(content)])
    chat.extend(tmp)
    return chat
